...
<body class="min-h-screen flex flex-col bg-base-200 font-sans">
        ...
        <div class="navbar-end gap-2">
            @auth
                ...
                <form method="POST" action="/logout" class="inline">
                    @csrf
                    <button type="submit" class="btn btn-ghost btn-sm">{{ __('general.logout') }}</button>
                </form>
            @else
                <a href="{{ route('login') }}" class="btn btn-ghost btn-sm">{{ __('general.signin') }}</a>
                <a href="{{ route('register') }}" class="btn btn-primary btn-sm">{{ __('general.signup') }}</a>
            @endauth
        </div>
    </nav>
    ...
    <footer class="my-24">
        <div class="flex justify-center gap-2 text-xs">
            <a class="text-gray-600! hover:underline" href="{{ route('lang', ['lang' => 'en']) }}">English</a>
            <a class="text-gray-600! hover:underline" href="{{ route('lang', ['lang' => 'fr']) }}">French</a>
        </div>
        <div class="mt-6 flex justify-center gap-2 text-xs text-gray-600">
            <span>{{ __('general.madeby') }}</span>
            <svg class="inline-block" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="25px" height="25px" viewBox="0 0 128 128" version="1.1">
                <g id="surface1">
